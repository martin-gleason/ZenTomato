...
| O19 | **The App Store name — "ZenTomato" is taken** *(blocks TestFlight)* | C9, C10 | App Store Connect returned `409 DUPLICATE.DIFFERENT_ACCOUNT` on the first upload attempt: the name is in use by another account. Costs nothing in code — bundle ID and home-screen name are unaffected — but no build can be uploaded until an app record exists, and no record can be created without a name. Effectively permanent once builds ship under it, and it is the name the public GPL repo carries, so it is `C10`'s decision too. |
