...
| O2  | **Migration over an existing install**                       | F6, F2c | `CachedTask.isRecurring` and `CompletedTaskRecord.wasRecurring` are the first schema change since F2. The 2026-08-24 build was installed over the old app; if the history survived, lightweight migration worked. **F2c makes this load-bearing rather than theoretical**: `AppSettings` gains a seventh column, so the next install over the owner's phone migrates a table with real settings in it. The column is optional so a pre-F2c row reads back as the system default — asserted in `AlertSoundTests`, but a unit test builds the old row rather than finding one. |
...
| O24 | **A block heard end-to-end on a chosen bell**                | F2c    | `F2c.md`'s *Done when*, and now actually runnable: choose a bell, run a block to its end, hear it; then turn Sound off, run another, hear nothing. **The bells have never been played on a device** — they were measured, trimmed and converted from a waveform, and a file that is correct in `afinfo` can still be inaudible through a phone speaker or unpleasant at the end of a focus block. The picker now has three entries, so this is a taste check as much as a function check. |
