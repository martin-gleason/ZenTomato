...
| O14 | **An explicit App ID with MusicKit, instead of the team wildcard** — *runbook: `docs/chores/C11.md`* | C8, C11 | The app is signed with `KH6NBQRZBY.*` and carries no MusicKit entitlement, so `MusicSubscription.current` can never succeed and the music row is permanently "couldn't be checked". Same root cause as O13. C2 asked for this App ID and it never landed. Playback still works; only the subscription check is blind. |
