...
| O23 | **The two CC0 bell files, supplied and trimmed**             | F2c    | `D25` chose two sounds; neither is in the tree. Freesound serves an HTML login page to a non-browser client — 17,034 bytes, HTTP 200, not audio — so the agent cannot fetch them and the owner must download both and drop them in `ZenTomato/Resources/` as `SmallBell.caf` and `StruckBell.caf`. **`Bell0005.WAV` is 41.7 s and must be trimmed to its first strike** (iOS caps a custom alert at 30 s); `afconvert` to CAF, as `Silence.caf` already is. Until then `AlertSound.playable` holds one entry, so the picker and its credits are hidden and the alarm is the system default — F2c is complete and inert, not broken. |
| O24 | **A block heard end-to-end on a chosen bell**                | F2c    | `F2c.md`'s *Done when*: choose a bell, run a block to its end, hear it; then turn Sound off, run another, hear nothing. Blocked on `O23`. The half that can be tested today — sound off is silent, sound on is the system default — is unchanged from build 5 and was verified there. |
