...
| O6  | **The alarm firing through an active Focus, and through silent mode** — **HALF ANSWERED 2026-08-27** | F2 | **Through a Focus: yes.** A full sprint was run with **Do Not Disturb on** and the alarm fired at every block end, including one on the lock screen. That is the half `SPEC.md` F2's whole design rests on — AlarmKit was chosen over a notification precisely so an alert would survive a Focus — and it holds. **Through silent mode: still untested, and not by oversight.** That sprint ran with the app's own sound setting **off**, which substitutes `Silence.caf`, so nothing could have made a noise whatever the ringer was doing. What remains is one block: **sound on, ringer switch off**. |
