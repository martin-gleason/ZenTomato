...
| ~~O26~~ | ~~**The alarm cannot be silenced from inside the app**~~ **BUILT 2026-08-28, unseen on a phone** | F2, F2c | `D26`, built as `F2d`. `AlarmScheduling` gains `alertingAlarmID`, `alertingUpdates()` and `stopAlerting(id:)`; the engine gains `silenceAlarm()`, which stops the noise and then runs the same `handleDismiss()` path `DismissBlockIntent` runs. Nine tests including a drift test that the button and the system alert produce the same recorded outcome. **Device check is `O29`.** |
...
| O29 | **The Silence button, on a phone**                           | F2d | `F2d`'s *Done when*, unrun. Two blocks: one ending with the app in the **foreground**, silenced from inside ZenPom without touching the system alert; one ending with the app closed, dismissed from the system alert as usual, to confirm the two paths do not fight — no double advance, no stuck button. **The placement is also for the owner's eye**: the Silence button sits above the primary control rather than replacing it, and pushes it down. Start and Stop are disabled while it shows, so what moves is inert. |
