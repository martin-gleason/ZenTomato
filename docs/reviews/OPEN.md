...
|-----|--------------------------------------------------------------|--------|--------------------------------------------------------------|
| O48 | **Whether the decisions register may carry a second `## Decisions` heading, and whether the status vocabulary is contract** | C34 | `docs/conventions.md` Axis 2 says *"one `##` per register"*, and `grep -n '^## Decisions' docs/plans/00-register.md` now returns two: `## Decisions — owner fields` (hand-maintained `P` and `TD`) and `## Decisions (D)` (generated from `00-deltas.md` per `D37`). No register is counted twice — the overlay heading carries no `(D)` symbol on purpose — but the shape is a reading of Axis 2 the agent chose, and `D37` said nothing about it. Second half: `C34` added `ratified` and `resolved` to `gen_status.CLOSED` and deliberately left `held` out, so `O37` stays counted open. That is a vocabulary ruling; Axis 2 pins only `proposed · ratified · rejected · superseded`. If the vocabulary is contract, `resolved` and `held` owe a `D<n>`. Raised by `C34`'s adversarial review as the agent's to propose and the owner's to ratify. |
| O47 | **A refused dedup read drops the wrist tap, and `Distraction.id` still has no uniqueness constraint** | F7b | `F7b` fixed the swallowed error: `try? context.fetch` mapped a refused read onto "nothing found" and fell through to the insert, so a WatchConnectivity resend could become a second row for one press. **The ruling shipped is DROP** — nothing is inserted and `Outcome.unreadable` is returned — because an over-count is permanent (no surface in this app can delete a row) while an under-count is merely absent. That is the owner's to overrule; it is one line. **The uniqueness constraint was declined, not forgotten:** SwiftData's `.unique` upserts rather than rejects, so a redelivered tap would overwrite a typed `note` with `nil` — *recorded as a claim, not as fact, because nothing here has run it* — and adding a constraint to a shipped model with no `VersionedSchema` risks the one store `O1` has never been read from. Taking it needs a `D<n>`, a versioned schema with an explicit de-dup migration, and an install-over-a-real-store check in `O2`'s shape. **Also unsettled and not this unit's:** a lost wrist tap is silent, while a refused tap on the phone gets an amber line, and `O15` cannot tell a tap lost to a refused read from one lost to WatchConnectivity. |
| O46 | **The v1.5 ratchet still cannot see `D33`, the delta it was built for** | C32 | `D41`'s second hole says *"nothing would ever have gone red"* for `D33`. After `C32` there is a ledger, a baseline file and a ratchet, and **`D33` still does not go red.** Half the cause was a defect and is fixed — it declared no document in its `Currently` block and was judged against `SPEC.md`; the declaration rule now reads a delta's preamble and `D33` resolves to `docs/specs/zenpom-v1.5.md`. The other half is not fixable by verbatim matching: `D33` amends an order **table** and quotes it as a comma list of ids, every one below the twelve-character fragment floor, so there is no verbatim overlap to find, and lowering the floor makes `F8` match everything. **Measured, not argued:** the pre-`C31` tree was reconstructed from `ac80738` and the finished detector run over it — the v1.5 owed set is `{D31}` and *"would this have gone red for `D33`?"* is **False**. So `D33` sits in `docs/specs/V15-AMENDMENTS-APPLIED.md` because a person put it there. The owner's to rule: accept that a paraphrasing delta is caught by the ledger and not the instrument, or ask for a different detector. |
| O45 | **Whether v1.5's amendment ledger belongs inside the baseline** | C32 | `docs/specs/zenpom-v1.5.md` is ratified and the agent may never edit it, so `C32` put its amendment ledger and baseline count in two files beside it — `docs/specs/V15-AMENDMENTS-APPLIED.md` and `docs/specs/V15-AMENDMENT-BASELINE.txt` — the same relationship `AMENDMENT-BASELINE.txt` already has to `SPEC.md`. `SPEC.md` carries its list inline because the **owner** edits `SPEC.md`. If the owner wants the same shape for v1.5 it is an edit to a ratified baseline and needs a `D<n>` of its own, exactly as `D40` was needed for `D33`. |
...
| ~~O2~~ | ~~**Migration over an existing install**~~ **CLOSED 2026-08-28** | F6, F2c | Build `202608281026` was installed over the running app, not a deleted one, and it carried F2c's seventh `AppSettings` column. The sprint ran to completion with settings and history intact, so lightweight migration works over a real store. This was the load-bearing one: a unit test builds a pre-F2c row, it cannot find one on a phone. |
| ~~O23~~ | ~~**The two CC0 bell files, supplied and trimmed**~~ **CLOSED 2026-08-28 — the record was wrong, not the work** | F2c | Filed saying the agent could not fetch the sounds and the owner must supply them. **The owner already had**: both WAVs were sitting untracked at the repository root, downloaded 2026-08-27, and the adversarial reviewer found them. The `curl` that returned a login page was real; the conclusion drawn from it was not checked against the tree. Both are now trimmed, normalised and committed as `SmallBell.caf` and `StruckBell.caf` — `docs/sounds/candidates.md` carries the exact commands. The raw 17 MB of WAV moved to the git-ignored `docs/sounds/sources/`, off the repository root where one `git add -A` would have committed it. |
| ~~O24~~ | ~~**A block heard end-to-end on a chosen bell**~~ **CLOSED 2026-08-28** | F2c | Sprint on build `202608281026`. Both bells heard at a block boundary; the owner's verdict: *"both sound good."* The trim points and levels in `docs/sounds/candidates.md` were chosen by measuring a waveform, and the measurement held. |
...
| ~~O26~~ | ~~**The alarm cannot be silenced from inside the app**~~ **BUILT 2026-08-28, unseen on a phone** | F2, F2c | `D26`, built as `F2d`. `AlarmScheduling` gains `alertingAlarmID`, `alertingUpdates()` and `stopAlerting(id:)`; the engine gains `silenceAlarm()`, which stops the noise and then runs the same `handleDismiss()` path `DismissBlockIntent` runs. Twenty-two tests across three suites, including a drift test that the button and the system alert produce the same recorded outcome. **Device check is `O29`.** |
| ~~O27~~ | ~~**Every setting is editable while a block runs**~~ **BUILT 2026-08-28** | F2c, F2e | `D27`, built as `F2e`. One `.disabled(isBlockRunning)` on a `Group` holding all three customization sections, with a fence that was shown to fail when a section is moved out of it. Device check is `O30`. |
| ~~O28~~ | ~~**The alert sounds need a preview**~~ **BUILT 2026-08-28** | F2c, F2e | `D28`, built as `F2e`. Selecting a sound plays it, which is iOS's own Settings › Sounds idiom rather than a new control. `Default` cannot be played — it is iOS's sound, not a file ZenPom holds — and the footer says so. Device check is `O30`, and the audio session is the half a simulator cannot answer. |
...
| ~~O3~~ | ~~**A full sprint with the screen locked, uninterrupted**~~ **CLOSED 2026-09-09** | F4 | Build `0.9.0 (202608281553)`, run as one uninterrupted sprint with `O4` and `O5` inside it. The owner: *"O3, O4, and O5 were one and succeeded. each passed the test."* The standing objection was precisely that the pieces had been seen individually and never as the single run `SPEC.md` describes. That run has now happened, and it closes F4's *Done when*. |
| ~~O4~~ | ~~**Headphones, CarPlay, and an incoming phone call**~~ **CLOSED 2026-09-09** | F4 | Same run. **This row was stale here, not in the register** — `00-register.md` already carried it closed and this file never got the strikethrough, which is exactly the second-intake-path failure `docs/conventions.md` warns about. One caveat kept rather than smoothed over: the owner reported the run passing as a whole and the three paths were not itemised, so this records one run, not three per-path results. |
| ~~O5~~ | ~~**A playlist short enough to reach its end, to prove looping**~~ **CLOSED 2026-09-09** | F4 | Same run, and same staleness as `O4`. A real playlist reached its end and looped — the half no test could ever reach, since the loop had only been asserted against a stand-in. |
| ~~O6~~ | ~~**The alarm firing through an active Focus, and through silent mode**~~ **CLOSED 2026-08-28** | F2 | **Both halves now observed.** Through a Focus: confirmed on three builds. Through silent mode: the owner ran the `C20` check on build `202608281026` — *"in silent and DND, the alarm consistently fired as did the sheet."* That is the half `SPEC.md` F2's whole design rests on: AlarmKit was chosen over a notification precisely so an alert would survive a Focus and the ringer switch, and it does. |
...
| ~~O21~~ | ~~**Music cannot be restarted during a break**~~ **CLOSED 2026-08-28 — built as `F4f`, verified on the device** | F4 | `D25` ratified and applied to `SPEC.md` line 27. All three of that line's obligations confirmed on hardware: a break starts silent even with music on, the switch turns it on for that break, and it stops at the boundary without being asked. The switch is scoped to the break — during one it shows and sets that block, because the standing intention already read "on" while nothing played, leaving nothing to switch. |
| ~~O7~~ | ~~**Sound off**~~ **CLOSED 2026-08-28 — on the phone**       | F2, C20 | AlarmKit has no silent case, so this setting rested entirely on a bundled silent file, and **nothing had ever observed a phone staying quiet**. It has now: *"bell sound OFF: no sound from phone."* `Silence.caf` does what the setting rests on. The watch is a separate question — `O33`. |
...
| ~~O10~~ | ~~**Apply the nine outstanding spec amendments**~~ **CLOSED 2026-08-27 — the count is zero** | C6 | `A1`–`A7` were applied 2026-08-24 and `A8`–`A9` on 2026-08-27; the row's "nine outstanding" had been stale since August. `AMENDMENT-BASELINE.txt` holds the authoritative count and `DeltaIntegrityTests` fails if it grows, so this is enforced rather than tracked. `docs/specs/AMENDMENTS-TO-APPLY.md` is now a record of what each amendment changed, not a to-do list. |
| ~~O11~~ | ~~**F7's gate: D2 into `SPEC.md`**~~ **CLOSED — D2 was applied 2026-08-24** | F7 | `docs/plans/F7.md` said *"Until D2 is merged into `SPEC.md`, this feature does not exist and no line of it may be written."* It was merged as part of `A2` on 2026-08-24, which is what unblocked F7 — the row simply outlived the fact, alongside `O10`. |
...
| ~~O22~~ | ~~**A CLA or copyright assignment, before the first outside pull request**~~ **CLOSED 2026-08-27 — `CONTRIBUTING.md` ratified by the owner** | C10, C18 | Ratified in its pledge-joining form: a contributor licenses their work GPL-3.0-or-later and joins the App Store non-enforcement pledge in `LICENSE-EXCEPTION.md`, keeping their copyright and every other GPL right. In force for every pull request opened after 2026-08-27; the instrument records its own effective date. What remains is enforcement at review, which is in the adversarial reviewer's brief. |
