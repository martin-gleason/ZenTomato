...
| ~~O26~~ | ~~**The alarm cannot be silenced from inside the app**~~ **BUILT 2026-08-28, unseen on a phone** | F2, F2c | `D26`, built as `F2d`. `AlarmScheduling` gains `alertingAlarmID`, `alertingUpdates()` and `stopAlerting(id:)`; the engine gains `silenceAlarm()`, which stops the noise and then runs the same `handleDismiss()` path `DismissBlockIntent` runs. Fifteen tests including a drift test that the button and the system alert produce the same recorded outcome. **Device check is `O29`.** |
...
| O29 | **The Silence button, on a phone** — **HALF ANSWERED 2026-08-28** | F2d | **App in the foreground: passes.** Build `202608281343`, the owner's words: *"with app open, silence button was amazing. all tests fired."* **Screen locked: the alarm and everything else fired, but the reflection sheet never appeared** — three external interruptions tapped and no way to write them down. That is `D29`, proposed not ratified: `reconcile()` refuses the prompt on the grounds that nobody was there, and a locked phone in a pocket is somebody being there. What remains for this item is the *second* half of the original check — a block ending with the app closed, dismissed from the system alert, confirming no double advance against the in-app button. |
...
| O31 | **The watch alarm's button says Stop, and ours says Done**   | F2d, F7 | Reported 2026-08-28: *"watch os stop app should be renamed silence."* **This app sets that label to `"Done"`** — `AlarmKitScheduler`'s `AlarmButton(text: "Done", …)` — and the watch app's own source has no Stop button at all, only the two capture buttons. So watchOS is drawing its own alert rather than honouring `AlarmButton`, and if so the word is not ours to change. **Not to be guessed at.** One check settles it: read what the *phone's* full-screen alert says. "Done" on the phone and "Stop" on the watch means watchOS overrides it and this closes as not-actionable; "Stop" on both means the label is being ignored everywhere and is worth chasing. |
...
| O7  | **Sound off** — **narrowed 2026-08-28, not closed**       | F2, C20 | AlarmKit has no silent case, so this setting rests entirely on a bundled silent audio file. `C20` came back *"the functionality is unchanged"* — **no defect**, which resolves it to `C20`'s mechanism A: settings freeze at block start, so a sound switched off mid-sprint does not silence the block already running. `D27` removes that confusion by locking the screen. **What is still not directly observed is a block run start to finish with the app's Sound setting off, staying quiet.** Recorded rather than assumed, because assuming exactly this is what put a wrong sentence in `O6` for a day. One minute of device time whenever convenient. |
