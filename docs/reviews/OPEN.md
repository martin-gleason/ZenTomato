...
| O30 | **The locked settings screen and the sound preview, on a phone** | F2e | `F2e`'s *Done when*, unrun. Three checks: the customization rows locked during a block with music and Todoist still live; **each bell previewed with the ringer switch off and a playlist running** — `.playback` with `.mixWithOthers` must be audible without stopping the music, and an audio session is the thing that behaves differently on a device than in a simulator; and a preview left mid-play by leaving the screen and by backgrounding the app, both of which must go quiet. |
