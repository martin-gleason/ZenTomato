...
| O27 | **Every setting is editable while a block runs, including the alert sound** | F2c | Reported 2026-08-28: *"I was able to change the sounds from small bell to struck bell in a sprint — but I shouldn't be able to do that."* **The alarm itself behaved correctly**: settings are frozen into `TimerSettingsSnapshot` at block start, so the change landed on the *next* block, which is what `SettingsView`'s own running-block note promises — *"Changes take effect when it ends, not now."* So this is a request to change ratified behaviour, not a defect, and it is app-wide rather than about one row. Owes a delta. |
| O28 | **The alert sounds need a preview**                          | F2c | Reported 2026-08-28. `F2c.md` ruled it out with a reason that still holds: *"playing an alarm inside Settings is a new audio path, `AlarmKit` does not offer it, and `AVAudioPlayer` for it would be a second sound system."* Choosing a sound you cannot hear is a real gap, and the owner has now hit it. Owes a delta, and the delta has to price the second audio path against `MusicCoordinator`. |
