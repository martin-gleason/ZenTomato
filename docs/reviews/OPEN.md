...
| O26 | **The alarm cannot be silenced from inside the app**          | F2, F2c | `cancelAlarm()` is reachable from two engine paths only — a confirmed Stop, and one internal path — and **no view observes `Alarm.State.alerting`**. The only way to end a ringing alarm is the system alert's own Dismiss button. Reported 2026-08-28: in the last short break, Stop did not stop, the sound continued and there was no way to end it. Two mechanisms fit and they have not been separated — see `docs/chores/C19.md`. **Not to be guessed at**: the F2b alarm arc cost four successive fixes, each introduced by the one before it. |
...
| O7  | **Sound off** — **CONTRADICTED ON A DEVICE 2026-08-28**   | F2, C20 | AlarmKit has no silent case, so one of the seven locked settings rests entirely on a bundled silent audio file — and **that had never once been observed on a phone**. It has now, and the observation went the wrong way: the owner ran a sprint with the app's Sound setting off and the alarm sounded anyway. Two mechanisms, not separated — see `docs/chores/C20.md`. Note that `O6`'s note asserted the opposite of this for a day: it said an earlier sprint proved nothing about silent mode *because* sound was off, "so nothing could have made a noise". That was an assumption written as an observation. |
