...
| ~~O26~~ | ~~**The alarm cannot be silenced from inside the app**~~ **BUILT 2026-08-28, unseen on a phone** | F2, F2c | `D26`, built as `F2d`. `AlarmScheduling` gains `alertingAlarmID`, `alertingUpdates()` and `stopAlerting(id:)`; the engine gains `silenceAlarm()`, which stops the noise and then runs the same `handleDismiss()` path `DismissBlockIntent` runs. Nineteen tests across three suites, including a drift test that the button and the system alert produce the same recorded outcome. **Device check is `O29`.** |
