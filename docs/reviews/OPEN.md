...
| O6  | **The alarm firing through an active Focus, and through silent mode** — **TESTED 2026-08-26, AND IT FAILED** | F2 | Field-tested in a real sprint for the first time: **no alarm sounds when the phone is silent.** This is not a nicety — `SPEC.md` F2 names it as the reason AlarmKit was chosen over a notification, and F2's whole value is a timer that tells you without being watched. Now a **defect against the contract**, not an untested assumption. Cause not yet established; see `A17`. |
...
| ~~O16~~ | ~~**Branch protection on `main` — it has never existed**~~ **CLOSED 2026-08-24** — PR required, CI required, linear history, rebase-only, no force pushes | C1 |
...
| O14 | **Does the music row give a real answer now?** — *now also readable in Settings → Music, which is the faster check* — *the App ID half is done; see `docs/chores/C11.md`* | C8, C11 | The App ID is explicit and MusicKit is enabled on it under App Services. The premise that a missing **entitlement** caused `ICError -7013` was wrong — there is no MusicKit entitlement for an iOS app. So whether those errors persist, and whether the subscription check now succeeds, is a device question rather than a settled one. | The app is signed with `KH6NBQRZBY.*` and carries no MusicKit entitlement, so `MusicSubscription.current` can never succeed and the music row is permanently "couldn't be checked". Same root cause as O13. C2 asked for this App ID and it never landed. Playback still works; only the subscription check is blind. |
...
| O17 | **Search in the music picker** — *needs a delta before it is built* | F4c | The other half of the note that produced F4c. A library of a few hundred playlists is unusable without one, but a search field is new functionality rather than a rearrangement, so it wants a delta argued rather than assumed. |
| O18 | **No `PrivacyInfo.xcprivacy`** — *not required today, worth having* | C12 | Verified rather than assumed: no required-reason API is used (the only `UserDefaults` in the tree is a word in a comment) and nothing is collected or sent, so Apple does not require a manifest. Worth adding anyway so "collects nothing" is machine-readable rather than a claim in a README. |
