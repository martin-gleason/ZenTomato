...
- **The stop condition is not a date.** v1.5 ends when **zenpom has driven four consecutive fortnightly Rhodia reviews from its own export** (`docs/specs/zenpom-v1.5.md`, ratified 2026-09-09). The September 13, 2026 hard stop that stood here was anchored to an exam that was cancelled, and it is gone. The real constraint is **review capacity** — one fixed afternoon slot at the 5% dial — which is why the order front-loads small PRs. Unmerged work at a milestone boundary returns to backlog, no forensics.
