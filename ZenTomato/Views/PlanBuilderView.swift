...
struct PlanBuilderView: View {
  ...
  ///
  /// **The work is in `PickerScreenModel.build(...)`, not here.** This file sat at
  /// the 400-line cap and `F13` added to it twice — a tint and a priority — so the
  /// construction moved to `PickerScreenModel+Build.swift`, beside the type it
  /// builds and reachable from a test with no screen behind it. The cap is what
  /// prompted the move and not what justifies it: turning database rows into plain
  /// values was never this view's job.
  private var picker: PickerScreenModel {
    PickerScreenModel.build(
      projects: Array(projects),
      sections: Array(sections),
      tasks: Array(tasks),
      completedThisSprint: completedThisSprint?.taskIDs)
  }
